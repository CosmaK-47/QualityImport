...
export default async function StaffResellersPage() {
  ...
  return <main className={styles.shell}>
    <header className={styles.header}><a href="/staff/orders" className={styles.brand}><span>QI</span><div><b>Quality Imports</b><small>Reseller operations</small></div></a><nav><a href="/staff/orders">Orders</a><a href="/staff/customers">Customers</a><a className={styles.active} href="/staff/resellers">Resellers</a>{role === "admin" && <a href="/staff/finance">Finance</a>}{role === "admin" && <a href="/staff/team">Team</a>}</nav><div className={styles.account}><small>{role}</small><b>{user.email}</b><a href={chatGPTSignOutPath("/")}>Sign out</a></div></header>
